...
fn log_params(p: &[f64], fp: &mut File, n_true_iter: usize) {
    for i in 0..n_true_iter {
        let mut line = "".to_owned();
        for j in 0..NGI {
            line.push_str(&format!("{} ", p[i*NPARAMS + j]));
        }
        for j in 0..NVIJ {
            line.push_str(&format!("{} ", p[i*NPARAMS + j + NGI]));
        }
        for j in 0..NFIJ {
            line.push_str(&format!("{} ", p[i*NPARAMS + j + NGI + NVIJ]));
        }
...
fn main() {
    ...
    let mut varparamsfp = File::create("varparams").unwrap();
